        ...
        case .loaded(let resource):
            ...
            case .generatedNewDeck(let questions):
                let questionsBinding = Binding(
                    ...
                    set: {
                        model.simplifiedDeckGenerationProcess = .loaded(
                            .generatedNewDeck(.init(topic: questions.topic, questions: $0))
                        )
                    }
                )
            ...
            case .savedDeck(let deck):
                ...
                set: {
                    model.simplifiedDeckGenerationProcess = .loaded(
                        .savedDeck(
                            .init(localId: deck.localId, topic: deck.topic, questionSet: $0)
                        )
                    )
                }
                )
                EditableLocalQuestionsListView(questions: questionsBinding)
            }
